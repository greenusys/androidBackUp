...
public class More_Notification extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_more__notification);

        final ProgressDialog pdLoading = new ProgressDialog(More_Notification.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        getSupportActionBar().setTitle("More Notification");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        rv_gd = (RecyclerView) findViewById(R.id.rv_mart);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.more_notification)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                More_Notification.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                                ...
                                try {
                                    ...
                                    if (ja.getString("status").equalsIgnoreCase("0")) {
                                        ...
                                        for (int i = 0; i < aaa.length(); i++)
                                        {
                                            ...
                                            soldire_gd_model soldire_gd_model1 = new soldire_gd_model(item.getString("merit_result_name"),item.getString("date"),item.getString("link"));
