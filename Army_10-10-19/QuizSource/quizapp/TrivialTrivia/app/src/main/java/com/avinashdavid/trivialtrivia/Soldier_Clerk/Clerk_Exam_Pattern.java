...
public class Clerk_Exam_Pattern extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_clerk_exam_pattern);

        getSupportActionBar().setTitle("Clerk Exam Pattern");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        final ProgressDialog pdLoading = new ProgressDialog(Clerk_Exam_Pattern.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        rv_gd = (RecyclerView) findViewById(R.id.rv_clerk_exam_pattarn);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.clerk_exam_pattern)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                Clerk_Exam_Pattern.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                                ...
                                try {
                                    ...
                                    if (ja.getString("status").equalsIgnoreCase("0")) {
                                        ...
                                        for (int i = 0; i < aaa.length(); i++)
                                        {
                                            ...
                                            Log.d("link", item.getString("exam_link"));
