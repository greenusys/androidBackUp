...
public class Clerk_Past_Paper extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_clerk_past__paper);

        getSupportActionBar().setTitle("Soldier Clerk Past Papers");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        final ProgressDialog pdLoading = new ProgressDialog(Clerk_Past_Paper.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        rv_gd = (RecyclerView) findViewById(R.id.rv_clerk_past_paper);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.clerk_past_paper)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                Clerk_Past_Paper.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
