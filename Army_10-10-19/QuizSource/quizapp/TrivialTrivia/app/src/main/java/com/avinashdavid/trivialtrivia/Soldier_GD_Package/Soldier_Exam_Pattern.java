...
public class Soldier_Exam_Pattern extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_soldier_exam_pattern);

        getSupportActionBar().setTitle("Soldier Exam Pattern");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        final ProgressDialog pdLoading = new ProgressDialog(Soldier_Exam_Pattern.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        rv_gd = (RecyclerView) findViewById(R.id.rv_soldier_gd_exam_pattarn);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.soldier_exam_pattern)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                Soldier_Exam_Pattern.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                                ...
                                try {
                                    ...
                                    if (ja.getString("status").equalsIgnoreCase("0")) {
                                        ...
                                        for (int i = 0; i < aaa.length(); i++)
                                        {
                                            ...
                                            String name = item.getString("exam_name");
                                            String link = item.getString("exam_link");
