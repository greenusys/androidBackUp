...
public class Soldier_syllabus_link extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_soldier_syllabus_link);


        getSupportActionBar().setTitle("Soldier Syllabus Link");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);


        final ProgressDialog pdLoading = new ProgressDialog(Soldier_syllabus_link.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        rv_gd = (RecyclerView) findViewById(R.id.rv_soldier_gd_syllabuslink);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.soldier_syllabus_link)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                Soldier_syllabus_link.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
