...
public class Soldier_Nursing_Past_Paper extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_soldier_nursing_past__paper);

        getSupportActionBar().setTitle("Soldier Nursing Past Papers");
        this.getSupportActionBar().setDisplayHomeAsUpEnabled(true);


        final ProgressDialog pdLoading = new ProgressDialog(Soldier_Nursing_Past_Paper.this);

        pdLoading.setMessage("Loading...");
        pdLoading.show();

        rv_gd = (RecyclerView) findViewById(R.id.rv_soldier_nursing_pastpaper);
        appController = (AppController) getApplicationContext();


        Request request = new Request.Builder()
                .url(URL.nursing_past_paper)
                .build();
        appController.getOkHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                Soldier_Nursing_Past_Paper.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                                ...
                                try {
                                    ...
                                    if (ja.getString("status").equalsIgnoreCase("0")) {
                                        ...
                                        for (int i = 0; i < aaa.length(); i++)
                                        {
                                            ...
                                            soldire_gd_model soldire_gd_model1 = new soldire_gd_model(item.getString("past_paper_name"),item.getString("date"),item.getString("link"));
