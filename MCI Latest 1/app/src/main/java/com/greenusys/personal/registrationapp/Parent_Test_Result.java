...
public class Parent_Test_Result extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_parent__test__result);
    ...
    public void API()
    {
        ...
        final ProgressDialog pdLoading = new ProgressDialog(Parent_Test_Result.this);
       ...
       // mRecyclerView=findViewById(R.id.subject_rv);
        ...
        RequestBody body= new FormBody.Builder().add("stud_id", "1").build();

        Request request = new Request.Builder().url("http://greenusys.website/mci/api/resultFetch.php").post(body).build();
        appController.getClient().newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                System.out.println("Errorrrrr " + e.getMessage());

                Parent_Test_Result.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                                ...
                                try {
                                    ...
                                    if (ja.getString("code").equalsIgnoreCase("1")) {
                                        ...
                                        for (int i = 0; i < aaa.length(); i++)
                                        {
                                            ...
                                            Log.d("result_excel_id", item.getString("result_excel_id"));
                                            Log.d("user_id", item.getString("user_id"));
                                            Log.d("user_name", item.getString("user_name"));
                                            Log.d("class_id", item.getString("class_id"));
                                            Log.d("subject_id", item.getString("subject_id"));
                                            Log.d("marks_obtained", item.getString("marks_obtained"));
                                            Log.d("total_marks", item.getString("total_marks"));
                                            Log.d("title", item.getString("title"));
                                            Log.d("date", item.getString("date"));
                                            Log.d("division", item.getString("division"));
